...
export async function generateCover({ slug, categoria, title }) {
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  try {
    const { path: out, sizeKB } = await generateCover({ slug, categoria, title });
    console.log(`Cover generada: ${out} (${sizeKB} KB)`);
  } catch (err) {
